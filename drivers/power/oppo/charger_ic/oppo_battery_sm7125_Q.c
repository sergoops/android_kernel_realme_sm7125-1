...
/*lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for QC charger*/
	smblib_dbg(chg, PR_MISC, "set USBIN_ALLOW_OVERRIDE: %d\n",
			allowed_voltage);
#else
	chg_err(" allowed_voltage: 0x%02x, rc=%d\n", allowed_voltage, rc);
#endif
	return rc;
}
...
/*lizhijie@BSP.CHG.Basic, 2020/02/25, lzj add for QC*/
	if(chip->vbatt_num == 2) {
		chg_err("smblib_set_adapter_allowance: 0x%02x\n", vbus_allowance);
		return 0;
	}
#endif
...
static const struct apsd_result *smblib_update_usb_type(struct smb_charger *chg)
{
	...
	if (chg->pd_active) {
		...
		oppo_set_smb5_usb_props_type(POWER_SUPPLY_TYPE_USB_PD);
#ifdef VENDOR_EDIT
/* lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for charger */
		chg_err("!!!smblib_update_usb_type: APSD=%s PD=%d\n", apsd_result->name, chg->pd_active);
#endif
...
/* lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for charger debug */
		chg_err("smblib_get_prop_usb_online false\n");
#endif
		return rc;
	}
...
/* lizhijie@BSP.CHG.Basic, 2020/02/25,  Add for get ibus current */
	...
	if (chip && chip->vbatt_num == 2 && chip->chg_ops->get_charger_current){
		...
		chg_err("ibus current: val->intval[%d]", val->intval);
...
/* ZhiJie.Li@PSW.BSP.CHG.Basic, 2020/02/25,  disable QC3 */
		...
		printk("!!! the hvdcp_detach_time:%lu\n", chg->hvdcp_detach_time);
...
/* ZhiJie.Li@PSW.BSP.CHG.Basic, 2020/02/25,  disable QC3 */
		chg->hvdcp_detach_time = cpu_clock(smp_processor_id()) / 1000000;
		chg_err("!!! the hvdcp_detach_time:%lu\n", chg->hvdcp_detach_time);
		if (!(chg->hvdcp_detect_ok && (chg->hvdcp_detach_time - chg->hvdcp_detect_time <= OPPO_HVDCP_DETECT_TO_DETACH_TIME))) {
			smblib_hvdcp_detect_enable(chg, false);
			chg->hvdcp_disable = true;
		} else {
...
#ifdef VENDOR_EDIT
/* lizhijie@BSP.CHG.Basic, 2020/02/25, lizhijie Add for charging */
    chg_err("!!!fg_oppo_set_input_current[%d]\n", fg_oppo_set_input_current);
...
#ifdef VENDOR_EDIT
/* lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for charging */
	chg_err("!!!IRQ: apsd-done rising; %s detected\n", apsd_result->name);
#endif
...
/*lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for svooc otg*/
	if (chg->typec_mode == POWER_SUPPLY_TYPEC_SINK
		&& chip->vbatt_num == 2 ) {
		chg_err("chg->typec_mode = SINK,Disable APSD!\n");
...
/* lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Add for charging */
	...
	if (dfp_status ^ current_status) {
		...
		chg_err("!!!!! smblib_handle_typec_cc_state_change: [%d], mode[%d]\n", dfp_status, chg->typec_mode);
	}
...
static void typec_disable_cmd_work(struct work_struct *work)
{
	...
	chg_err("!!! re-active t-c module\n");

	msleep(200);
	if (smblib_get_prop_typec_mode(chg) == POWER_SUPPLY_TYPEC_NONE) {
		chg_err("!!! fake typec plug\n");
...
static void oppo_chg_monitor_work(struct work_struct *work)
{
	...
	if (counts >= (chip->batt_full ? 8 : 3)) {//because rechg counts=6
		...
		if (get_client_vote(chg->usb_icl_votable, BOOST_BACK_VOTER) == 0
			...
			chg_err("oppo_chg_monitor_work: boost back\n");
			if (chg->wa_flags & BOOST_BACK_WA)
				vote(chg->usb_icl_votable, BOOST_BACK_VOTER, false, 0);
		}
		...
		if (stat & PRE_TERM_BIT) {
			...
			chg_err("PRE_TERM_BIT is set[0x%x], clear it\n", stat);
...
int oppo_mcu_id_gpio_init(struct oppo_chg_chip *chip)
{
	...
	chg_err("successful get mcu id default");
...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	if (rc < 0)
		return rc;
#ifdef VENDOR_EDIT
	smblib_get_iio_channel(chg, "usb_temp_adc", &chg->iio.usbtemp_v_chan);
	smblib_get_iio_channel(chg, "usb_supplementary_temp_adc", &chg->iio.usbtemp_sup_v_chan);
#endif
...
static void oppo_set_otg_switch_status(bool value)
{
	...
	if (vbus_rising == true) {
		...
		#ifdef ODM_LQ_EDIT
		/*nanxiaoyong@ODM_LQ@BSP.Tcard,2020/06/05,modified  for Turn off the OTG switch, otg_switch has not changed */
		if(!(g_oppo_chip->otg_switch == true && g_oppo_chip->otg_online == true && g_oppo_chip->vbatt_num == 2
			&& g_oppo_chip->ui_otg_switch == false)) {
			return;
		}
		#endif
	}
...
/* lizhijie@BSP.CHG.Basic, 2020/02/25, lzj Modify for charging */
		...
		if (g_oppo_chip && g_oppo_chip->vbatt_num == 2 && oppo_chg_is_usb_present()) {
...
/* lizhijie@BSP.CHG.Basic. 2020/02/25 lzj  add  for c to c */
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_PD && oppochg_pd_sdp == true) {
			val->intval = 1;
			break;
		}
#endif
...
/* Ji.Xu PSW.BSP.CHG  2018-07-23  Save battery capacity to persist partition */
	case POWER_SUPPLY_PROP_BATTERY_INFO:
		if (chg->batt_info_id < 0 || chg->batt_info_id >= BATT_INFO_MAX) {
			chg_err("ERROR  POWER_SUPPLY_PROP_BATTERY_INFO\n");
			return 0;
		}
...
#ifdef VENDOR_EDIT
/* lizhijie@BSP.CHG.Basic, 2020/02/25,   lzj dd for cts */
		if (g_oppo_chip) {
		...
		const union power_supply_propval *val)
{
	...
	/* Ji.Xu PSW.BSP.CHG  2018-07-23  Save battery capacity to persist partition */
	case POWER_SUPPLY_PROP_BATTERY_INFO:
		chg_err("POWER_SUPPLY_PROP_BATTERY_INFO, val->intval[%d]\n", val->intval);
		rc = fg_set_battery_info(chg, val->intval);
		break;
	case POWER_SUPPLY_PROP_BATTERY_INFO_ID:
		chg_err("POWER_SUPPLY_PROP_BATTERY_INFO_ID, val->intval[%d]\n", val->intval);
		chg->batt_info_id = val->intval;
		break;
	case POWER_SUPPLY_PROP_SOC_NOTIFY_READY:
		g_oppo_chip->healthd_ready = val->intval;
		chg_err("healthd running, reset 5s_thread\n");
		oppo_chg_wake_update_work();
		break;
#endif
	case POWER_SUPPLY_PROP_STATUS:
...
/* lizhijie@BSP.CHG.Basic, 2020/02/25, lizhijie Modify for charging */
	...
	chg_err("SMB5 status - usb:present=%d type=%d batt:present = %d health = %d charge = %d\n",
