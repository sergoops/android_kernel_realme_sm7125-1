...
#Now Its time for other stuffs like cloning, exporting, etc
 ...
 clone() {
    ...
    then
        msg "|| Cloning libufdt ||"
        git clone --depth=1 https://android.googlesource.com/platform/system/libufdt "$KERNEL_DIR"/scripts/ufdt/libufdt
    fi
}
...
exports() {
    export KBUILD_BUILD_USER="sergoops"
    export ARCH=arm64
...
setversioning() {
    # For staging branchc
    if [ $LTO = "1" ];then
        KERNELNAME="X2custom-$COMPILER-LTO-nodim-$DATE"
    else 
        KERNELNAME="X2custom-$COMPILER-nodim-$DATE"
    fi
